...
//服务器证书 SS.cert.pem
#define SERVER_SIGN_CERT_FILE 	"mk_tls_cert/sm2Certs/SS.cert.pem"
//服务器私钥
#define SERVER_SIGN_KEY_FILE 	"mk_tls_cert/sm2Certs/SS.key.pem"
//服务器加密证书SE.cert.pem
#define SERVER_ENCODE_CERT_FILE "mk_tls_cert/sm2Certs/SE.cert.pem"
//服务器加密私钥
#define SERVER_ENCODE_KEY_FILE 	"mk_tls_cert/sm2Certs/SE.key.pem"
 
using namespace std;
static int SetSocketReuseAddr(int socket, bool enable)
{
...
int TcpInit()
{
    ...
    do{
        ...
        if( ::listen(listener, 5) < 0)
            break;
		SetSocketReuseAddr(listener, true);
        return listener;
    }while(0);
...
{  
    ...
    // 加载自己的证书  
    if(SSL_CTX_use_certificate_file(ctx, SERVER_SIGN_CERT_FILE, SSL_FILETYPE_PEM) <= 0)
    {
    ...
    if(SSL_CTX_use_PrivateKey_file(ctx, SERVER_SIGN_KEY_FILE, SSL_FILETYPE_PEM) <= 0)
    {
    ...
    if(SSL_CTX_use_certificate_file(ctx, SERVER_ENCODE_CERT_FILE, SSL_FILETYPE_PEM) <= 0)
    {
        printf("SSL_CTX_use_certificate_file error!\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }
    if(SSL_CTX_use_PrivateKey_file(ctx, SERVER_ENCODE_KEY_FILE, SSL_FILETYPE_PEM) <= 0)
    {
        printf("SSL_CTX_use_PrivateKey_file error!\n");
        ERR_print_errors_fp(stderr);
        return -1;
    ...
    strcpy(szBuffer, "server: I am server and I have recieved your request!\n");  
